...
let showArticles = COUNT_SHOW_ARTICLES_CLICK;
...
async function getArticles() {
  try {
    ...
    if ((articlesData.length > COUNT_SHOW_ARTICLES_CLICK) &&
...
function renderStartPage(data) {
  ...
  const arrArticles = data.slice(0, COUNT_SHOW_ARTICLES_CLICK);
...
function sliceArrArticles() {
  ...
  const countShowArticles = COUNT_SHOW_ARTICLES_CLICK * countClickBtnShowArticles;
...
navButton.onclick = () => {
  if (nav.classList.toggle('open')) {
      ...
      navButtonImage.src = "http://localhost/NRusNews/img/icons/nav-open.svg";
  }
}